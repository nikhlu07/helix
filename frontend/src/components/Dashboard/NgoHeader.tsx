...
const roleDisplay: Record<string, { icon: string; name: string }> = {
  ngo_head: { icon: '👑', name: 'NGO Head' },
  ngo_program_manager: { icon: '📈', name: 'Program Manager' },
  ngo_field_officer: { icon: '🏃', name: 'Field Officer' },
  ngo_volunteer_coordinator: { icon: '❤️', name: 'Volunteer Coordinator' },
  ngo_admin: { icon: '⚙️', name: 'NGO Admin' },
...
export function NgoHeader({ user, onLogout }: HeaderProps) {
  ...
  const currentUser = ctxUser || user || { role: 'ngo_head', sector: 'ngo', isAuthenticated: false };
  const currentRoleName = (roleDisplay[currentUser.role] || roleDisplay.default).name;
  
  const getRoleFromPath = () => {
    if (location.pathname.includes('/ngo-head')) {
      return { name: 'NGO Head', role: 'ngo_head' };
    } else if (location.pathname.includes('/ngo-program-manager')) {
      return { name: 'Program Manager', role: 'ngo_program_manager' };
    } else if (location.pathname.includes('/ngo-field-officer')) {
      return { name: 'Field Officer', role: 'ngo_field_officer' };
    } else if (location.pathname.includes('/ngo-volunteer-coordinator')) {
      return { name: 'Volunteer Coordinator', role: 'ngo_volunteer_coordinator' };
    } else if (location.pathname.includes('/ngo-admin')) {
      return { name: 'NGO Admin', role: 'ngo_admin' };
    }
